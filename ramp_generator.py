...
class RampGenerator:
    ...
    def ramp(self):
        ...
        for i in range(n_dims):
            t1 = torch.linspace(0, self.height[i], steps=self.n_ramp[i], dtype=torch.float64, device=self.device)
            t2 = torch.linspace(self.height[i], self.height[i], steps=self.n_steps - 2 * self.n_ramp[i],
                                dtype=torch.float64, device=self.device)
            t3 = torch.linspace(self.height[i], 0, steps=self.n_ramp[i], dtype=torch.float64, device=self.device)
