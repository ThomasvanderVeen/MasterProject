...
parameters = Parameters(max_joint_angle=1, min_joint_angle=1, n_hairs=1, t_total=3, dt=0.0001, n_angles=1)
parameters.sensory['n'] = 5
t_total = 3
colors = ['green', 'yellow', 'blue', 'black', 'red']
spike_rates, spike_rates_list = [], []
times, times_list = [], []
I_list = np.linspace(18, 18, num=2, dtype=int) * 1E9
b_list = np.linspace(264, 264, num=2, dtype=int) * 1e-12
MSE = np.zeros((I_list.size, b_list.size))

for l, m in itertools.product(range(I_list.size), range(b_list.size)):
...
I_opt, b_opt = I_list[indexes[0]], b_list[indexes[1]]
...
df = pd.DataFrame(MSE.astype(int), columns=np.round(b_list*1E12), index=np.round(I_list, 2)*1E-9)
#plot_heat_map(df)

print(f'[Single hair] Optimum I {I_opt}, Optimum b: {b_opt}')
