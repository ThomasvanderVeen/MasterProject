...
parameters = Parameters(t_total=5.5, dt=0.0001)
...
for t in tqdm(tau):
    ...
    for i in range(N_simulations):
        pitch = np.array(data[f'simulation_{i}'][2][2, :])
...
plt.scatter(range(len(train)), train)
plt.scatter(range(len(train)), test)
plt.show()

plt.plot(tau, y_predicted, y_test)
plt.show()